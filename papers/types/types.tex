...
communicated, but also in which direction.  We \remph{declare} an actor
by naming it and specifying the protocol it follows. For
example, we may declare a type checking actor
\[
...
check@p = p?ty. p?tm. case tm    -- inputs type and term via p. match on tm.
  ...
  ; ['Emb e] ->                  -- embedded synthesisable term e:
      synth@q. q!e. q?S.         -- spawn a child to synthesise type S for e,
      S ~ ty }                   -- unify S and ty.
\end{lstlisting}
%
\begin{lstlisting}[columns=fullflexible,keepspaces=true]
...
whether a metavariable has been solved, but they can \remph{block} when
