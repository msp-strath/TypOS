...
communicated but also in which direction. We \emph{declare} an actor by
giving it a name, and specifying the protocol it follows E.g., we may
...
\[
  \tt synth \hab \query\atom{Synth}\then \query\atom{Type}\then
\]
The typing rules are then given by \emph{defining} those actors to
perform the communications correspoing to the rules' conclusions. An
