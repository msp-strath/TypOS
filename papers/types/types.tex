...
\authorrunning{G. Allais, M. Altenm\"uller, F. Nordvall Forsberg, C. McBride, G. Nakov, and C. Roy}
\titlerunning{\TypOS}
...
concurrent actors, but sharing a store of metavariables which become
monotonically more defined, as is typical in type inference.
We tackle typing tasks not with a single process
traversing a syntax tree, but rather by spawning a network of processes, each
responsible for one node of the syntax tree, communicating with the
processes for its parent and child nodes via channels. The notion of
\emph{judgement form} is recast as channel \emph{interaction
  protocol}, specifying not only what is to be communicated but also
in which direction. E.g., we may readily distinguish type checking
actors, which input a type and a term then succeed or fail, from type
synthesis actors, which input a term then output a type or fail.
Where a typing rule invokes premises, a typing actor spawns children.
...
careful to distinguish these `actor variables' from the `term
variables' of the syntax being manipulated by the actors, for which we
take generic LISP-style atoms and pairs and add a distinct
$\alpha$-invariant notion of term variables and binding. Schematic
...
action, or in a pattern-match, and zero or more use sites. The term
variables in scope for each actor variable are thus determined
precisely. Likewise, the term variables in scope at the time a channel
is created limit what is in scope for the signals on the channel.
...
any variables which were already free when they were spawned, and can
thus take no action which might violate stability under substitution.
...
check@p = p?ty. p?tm. case tm    -- inputs ty and tm via p. match on tm.
  { ['Lam \x. body] -> ?S. ?T.   -- lambda case: ask for inputs S and T.
  ...
  ; ['Emb e] -> synth@q. q!e. q?S. S ~ ty -- embedded term e:
                                          -- synthesize type S for e in child,
                                          -- unify S and ty.
  }
\end{verbatim}
