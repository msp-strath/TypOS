...
\TypOS{} supports a generic syntax for terms with LISP-style atoms (\atom{\textit{name}}) and
pairs (\pair{\textit{s}}{\textit{t}}) and a distinct
$\alpha$-invariant notion of term variables ($x$) and binding
($\bind x t$).
...
variables' of the syntax being manipulated by the actors. Schematic
...
constructing dependent pairs or applying depedent functions, and
observe that their algorithmic status happens more by luck than
judgement.
