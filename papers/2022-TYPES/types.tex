...
For example, we may declare a type checking actor
\[
...
\]
which receives a synthesisable term, then outputs a type or fails.

\paragraph{Typing rules}
Typing rules are then given by \remph{defining} actors to
...
check@p = p?ty. p?tm. case tm  -- inputs type and term via p. match on tm
  { ['Lam \x. body] -> ?S. ?T. -- lambda case: make fresh metavars S and T
      ( ty ~ ['Arr S T]        --   unify ty with arrow type, fork in parallel:
      | \x.                    --   bind fresh x (now in scope for children)
        synth { x -> S }.      --   link S with x for descending synth actors
        check@q. q!T. q!body.) --   spawn child on channel q to check body
...
synth@p = p?tm.                -- input term tm via p
  lookup tm { S -> p!S. }      -- try to lookup tm in synth context
   ...
   { ['Ann t ty] ->            -- type annotated term: in parallel
       ...
       | p!ty. )               --   send type ty
   ; ['App f s] -> ?U. ?V.     -- application: make fresh metavars U and V
       ( synth@q. q!f. q?ty.   --   spawn child to synthesise a type ty for f
         ty ~ ['Arr U V]       --   unify ty with arrow type
       | check@r. r!U. r!s.    --   spawn child to check argument
       | p!V. ) }              --   send the target type back
\end{lstlisting}
...
concurrent processes may safely share~\cite{kuper2015theses}: actors may create and share \emph{metavariables}, which mutate only by becoming
defined when a unification constraint is solved.
%
Metavariables thus provide a
secondary means for actors to communicate. Actors cannot detect
whether a metavariable has been solved, but they \remph{block} when
they match strictly on a metavariable which is currently unknown.
Decisions taken on the basis of less information need
...
Actors can restart upon the update of blocking metavariables in the
global unification state, where information increases monotonically.
The final configuration of the stack represents a typing derivation
of the term.
