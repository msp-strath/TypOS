...
{-
--  foo : forall {ga de xi} -> ga %>^ de -> xi %>^ de -> (ga <<< xi) %>^ de

  padPrefix : forall {ga de xi} -> ga %>^ (xi <<< de) -> (xi <<< ga) %>^ (xi <<< de)
  padPrefix {de = de} ([] & th) = {!!} --ok
  padPrefix (_-,_ {de = []} sg x & th) = {!!}
  padPrefix (_-,_ {de = de -, x₁} sg x & th) = {!!}
  padPrefix (x -/ x₁ & th) = {!!}
-}
